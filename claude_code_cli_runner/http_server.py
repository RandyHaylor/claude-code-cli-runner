    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/auth-status":
                ...
                self.wfile.write(body)
                return
            # Version handshake: callers enforce a required runner version
            # so deployment skew is loud, never silent. Unauthenticated —
            # it reveals only the code version.
            if self.path == "/version":
                from claude_code_cli_runner import RUNNER_CODE_VERSION
                body = json.dumps({"runner_version": RUNNER_CODE_VERSION}).encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Content-Length", str(len(body)))
                self.end_headers()
