...
def test_send_command_is_injected_and_echoed(tmp_path):
    # Queue a send_command DURING the run (the channel is transport for exactly
    # one run — commands written before run start are cleared at launch); the
    # stub echoes injected stdin messages back into stdout, so it shows up in
    # the live log.
    os.environ["STUB_RUN_FOREVER"] = "1"

    def queue_send_command_then_end():
        time.sleep(0.2)
    ...
    injector = threading.Thread(target=queue_send_command_then_end)
    injector.start()
    try:
        request = RunRequest(
...
def test_stale_control_command_from_previous_run_is_cleared_at_run_start(tmp_path):
    # A stale end_and_return left over from a PREVIOUS run (e.g. a manual stop)
    # must never end the next run: the channel is cleared at run start.
    append_control_intent(str(tmp_path), CONTROL_END_AND_RETURN)
    request = RunRequest(
    ...
    result = run_claude_code_task(request, build_command=stub_build_command)
    assert result.operator_ended is False
    assert result.final_result_event is not None
    with open(control_channel_path(str(tmp_path)), encoding="utf-8") as handle:
        assert handle.read() == ""
